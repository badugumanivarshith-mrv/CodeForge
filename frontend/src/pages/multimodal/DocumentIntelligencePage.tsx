...
export const DocumentIntelligencePage: React.FC = () => {
  const [name, setName] = useState('financial_ledger_q3.pdf');
  const [storageUrl, setStorageUrl] = useState('https://storage.googleapis.com/codeforge-assets/ledger.pdf');
  const [fileSize, setFileSize] = useState(85000);
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const outcome = await multimodalApi.analyzeDocument({
        name,
        storageUrl,
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-indigo-900/40 pb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Parse structural PDF layout pages, categorize metadata fields, and verify extracted ledger contents.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <form onSubmit={handleAnalyze} className="lg:col-span-2 space-y-6 bg-slate-900/40 border border-slate-900 p-8 rounded-2xl shadow-xl">
          <div className="space-y-1">
            <label className="text-xs font-semibold uppercase text-slate-400">Document File Name (.pdf)</label>
            <input
              type="text"
              required
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold uppercase text-slate-400">Estimated Character Size</label>
            <input
              type="number"
              required
              className="w-full bg-slate-950 border border-slate-850 focus:border-indigo-500 rounded-lg p-3 text-sm text-slate-100 font-medium transition-all focus:outline-none"
          ...
          >
            {loading ? 'Executing PDF Layout Parser...' : '📄 Run Document Analysis ➔'}
          </button>
        </form>

        <div className="p-6 rounded-2xl bg-slate-900/40 border border-slate-900 shadow-2xl h-fit space-y-4">
          <h2 className="text-lg font-bold text-slate-200">Extraction Output</h2>
          {result ? (
            <div className="space-y-4 text-xs font-mono">
              ...
              <div>
                <span className="text-[10px] text-slate-500 block">Parser Confidence</span>
                <span className="text-sm font-bold text-emerald-400 mt-0.5 block">{result.confidenceScore * 100}%</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 block">Categorized Document Tags</span>
              ...
              {result.ocrText && (
                <div>
                  <span className="text-[10px] text-slate-500 block">Parsed Document Content</span>
                  <p className="text-[11px] text-slate-300 leading-relaxed mt-1">{result.ocrText}</p>
                </div>
              )}
            </div>
          ) : (
            <p className="text-xs text-slate-500 leading-normal">Submit a document URL to parse text contents and tabular representations.</p>
          )}
        </div>
      </div>
    </div>
